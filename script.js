...
// --- API Layer for User Data (Cloudflare Worker) ---
const cloudflareApi = {
...
MAIN_AUTH_SUBMIT_BTN.addEventListener('click', async (e) => {
    ...
    if (isCreateMode) {
        ...
        const response = await cloudflareApi.register(email, password, minecraftUsername, accountName, minecraftEdition);
...
PROFILE_FORM.addEventListener('submit', async (e) => {
    ...
    const response = await cloudflareApi.updateProfile(currentUser.email, updatedProfile);
...
CHANGE_PASSWORD_FORM.addEventListener('submit', async (e) => {
    ...
    const response = await cloudflareApi.changePassword(currentUser.email, currentPassword, newPassword);
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    const storedAuthData = sessionStorage.getItem('current_auth_email');
    if (storedAuthData) {
        try {
            const { email, password } = JSON.parse(storedAuthData);
            // Attempt auto-login with stored creds
            cloudflareApi.login(email, password).then(response => {
                if (response.success) {
                    currentUser = { email: response.email, uid: response.uid };
                    userProfile = response.user;
                    handleSuccessfulAuth();
